...
//Actualización del nombre de ventana
   std::ostringstream ss;
   ss << "Proyecto Nicolás | FOV " << std::fixed << std::setprecision(2) << camera.fovy
      << " | Sens " << std::fixed << std::setprecision(2) << camera.mouseSensitivity
      << " | Speed " << std::fixed << std::setprecision(2) << camera.speed;
   ...
   glm::mat4 P = glm::perspective(glm::radians(camera.fovy), aspect, nplane, fplane);

   // Matriz V
   glm::mat4 V = camera.getView();
   shaders.setVec3("ueye", camera.pos);
